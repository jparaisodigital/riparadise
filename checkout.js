...
function goToMessengerWithOrder(order) {
    ...
    // 1. Always copy first (most reliable)
    copyToClipboard(text);

    // 2. Then try to open Messenger (with prefill attempt)
    setTimeout(() => {
        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

        if (isMobile) {
            window.location.href = urlWithText;
        } else {
...
function copyToClipboard(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text)
            .then(() => showToast('Order details copied — paste in Messenger'))
...
function fallbackCopy(text) {
    ...
    try {
        ...
        showToast('Order details copied — paste in Messenger');
    } catch (e) {
...
function showSuccess(order) {
    ...
    if (order.payment === 'cod') {
        instructions.innerHTML =
            'We prepared your order details.<br>Tap the button below to <strong>copy them</strong> and open Messenger.';
    } else {
        ...
        instructions.innerHTML = `
            ...
            <p style="margin-top:14px; line-height:1.5;">
                1. Scan & pay via GCash<br>
                2. Tap the button below to <strong>copy order details</strong> and open Messenger<br>
                3. Paste the details + attach your GCash receipt
            </p>
        `;
    }
    ...
    if (messengerBtn) {
        ...
        messengerBtn.textContent = 'Copy Details & Open Messenger';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Success modal — Back to Shop
    const successBackLink =
        document.getElementById('success-back-link') ||
